...
$boQua = 'i18n/dict|FormDocHeader|requests/\[id\]|purchasing/\[id\]|reports/page|lashing/\[id\]|node_modules|/\.next/|/scripts/'
...
foreach ($f in $tep) {
    ...
    foreach ($dong in [IO.File]::ReadAllLines($f.FullName)) {
        $i++
        $t = $dong.Trim()
        if ($t -match '^(//|\*|/\*|\{/\*)' -or $t -match '\*/\}?$') { continue }   # comment (ca dong cuoi cua comment nhieu dong)
        if ($t -match $mau) {
            $n++
